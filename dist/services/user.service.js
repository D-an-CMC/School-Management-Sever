...
async function generateStudentCode(schoolYearId) {
    const { data: sy } = await supabase
        .from('school_years')
        .select('year_name')
        .eq('school_year_id', schoolYearId)
        .single();
    const yearName = sy?.year_name || String(new Date().getFullYear());
...
async function generateTeacherCode() {
    const { data } = await supabase
        ...
        .limit(1);
    const raw = data?.[0]?.teacher_code || 'GV0';
...
export class UserService {
    ...
    async createUser(input) {
        ...
        let teacherCode = input.teacher_code;
        let schoolYearIdForStudent;
        if (input.role === 'HocSinh-PhuHuynh' && !studentCode && input.class_id) {
            ...
            if (!schoolYearIdForStudent) {
                ...
                schoolYearIdForStudent = classQuery.data?.school_year_id;
            }
            if (schoolYearIdForStudent) {
        ...
        return success({
            ...finalUser,
            role_name: finalUser.roles?.role_name || input.role,
        });
    }
    async updateUser(userId, patch) {
        ...
        if (patch.username !== undefined)
            updateData.username = patch.username;
        if (patch.phone !== undefined)
            updateData.phone = patch.phone;
        if (patch.is_active !== undefined)
            updateData.is_active = patch.is_active;
        if (patch.password !== undefined)
            updateData.password = patch.password;
        if (patch.role !== undefined) {
            const { data: roleRow } = await supabase.from("roles").select("role_id").eq("role_name", patch.role).single();
            if (roleRow)
                updateData.role_id = roleRow.role_id;
        }
        ...
        if (patch.full_name !== undefined || patch.gender !== undefined || patch.date_of_birth !== undefined || patch.class_id !== undefined || patch.student_code !== undefined) {
            ...
            if (patch.full_name !== undefined)
                studentData.full_name = patch.full_name;
            if (patch.gender !== undefined)
                studentData.gender = patch.gender;
            if (patch.date_of_birth !== undefined)
                studentData.date_of_birth = patch.date_of_birth;
            if (patch.class_id !== undefined)
                studentData.class_id = patch.class_id;
            if (patch.student_code !== undefined)
                studentData.student_code = patch.student_code;
            const { error: studentError } = await supabase.from("students").update(studentData).eq("user_id", userId);
            if (studentError)
                return errResp(studentError.message, "UPDATE_STUDENT_FAILED");
        }
        if (patch.full_name !== undefined || patch.gender !== undefined || patch.date_of_birth !== undefined || patch.phone !== undefined || patch.teacher_code !== undefined || patch.department !== undefined) {
            const teacherData = {};
            if (patch.full_name !== undefined)
                teacherData.full_name = patch.full_name;
            if (patch.gender !== undefined)
                teacherData.gender = patch.gender;
            if (patch.date_of_birth !== undefined)
                teacherData.date_of_birth = patch.date_of_birth;
            if (patch.phone !== undefined)
                teacherData.phone = patch.phone;
            if (patch.teacher_code !== undefined)
                teacherData.teacher_code = patch.teacher_code;
            if (patch.department !== undefined)
                teacherData.department = patch.department;
            const { error: teacherError } = await supabase.from("teachers").update(teacherData).eq("user_id", userId);
            if (teacherError)
                return errResp(teacherError.message, "UPDATE_TEACHER_FAILED");
        }
    ...
    async getStats() {
        ...
        const adminRoleId = rolesData?.find((r) => r.role_name === 'Admin')?.role_id;
        const studentRoleId = rolesData?.find((r) => r.role_name === 'HocSinh-PhuHuynh')?.role_id;
        const teacherRoleId = rolesData?.find((r) => r.role_name === 'GiaoVien')?.role_id;
        let teacherCount = 0;
        if (teacherRoleId) {
