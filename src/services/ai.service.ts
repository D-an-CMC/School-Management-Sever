...
export class AiService {
  ...
  async riskWarnings(params: { classId?: number; studentId?: number }) {
  ...
  async learningPredictions(studentId: number) {
    const result = await supabase
      .from('learning_predictions')
      .select('*')
  ...
  async chatbotConversations(userId: number) {
    const result = await supabase
  ...
  async addMessage(conversationId: number, message: { sender_type: string; message_content: string }) {
    const result = await supabase
